...
func InitSkyWalking() {
	...
	var err error
	cfg := Config()
	GReporter, err = reporter.NewGRPCReporter(cfg.Reporter.LocalAgentHostPort)
	if err != nil {
		log.Errorw(log.NameNet, "InitSkyWalking NewGRPCReporter",
	...
	//抽样
	opts = append(opts, go2sky.WithSampler(cfg.Sampler.SamplingRate))
	GTracer, err = go2sky.NewTracer(cfg.ServiceName, opts...)
	if err != nil {
		log.Errorw(log.NameNet, "InitSkyWalking NewTracer",
			log.KNameCommonErr, err)
		return
	}
}
...
func EntrySpan(peer string, operationName string, layer v3.SpanLayer) (span go2sky.Span, subCtx context.Context, err error) {
	...
	if err != nil {
		log.Errorw(log.NameLogic, "Tracer CreateEntrySpan",
			log.KNameCommonErr, err,
			log.KNameCommonData, peer+operationName,
		)
		return span, ctx, err
	}
...
func LocalSpan(ctx context.Context, peer string, operationName string, layer v3.SpanLayer) (span go2sky.Span, subCtx context.Context, err error) {
	...
	if err != nil {
		log.Errorw(log.NameLogic, "Tracer CreateLocalSpan",
			log.KNameCommonErr, err,
			log.KNameCommonData, peer+operationName,
		)
		return span, ctx, err
...
func ExitSpan(ctx context.Context, peer string, operationName string, layer v3.SpanLayer) (span go2sky.Span, err error) {
	...
	if err != nil {
		log.Errorw(log.NameLogic, "Tracer CreateExitSpan",
			log.KNameCommonErr, err,
			log.KNameCommonData, peer+operationName,
		)
		return span, err
...
func ExitSpanGRpc(ctx context.Context, peer string, operationName string, layer v3.SpanLayer) (span go2sky.Span, ctxValue context.Context, err error) {
	...
	if err != nil {
		log.Errorw(log.NameLogic, "Tracer ExitSpanGRpc",
			log.KNameCommonErr, err,
			log.KNameCommonData, peer+operationName,
		)
		return span, ctxValue, err
