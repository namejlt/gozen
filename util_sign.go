...
func UtilSignCheckSign(c *gin.Context, token string) bool {
	...
	switch signVer {
	case SignVersionOld:
		return checkSign(c, signCookie, SignVersionOld, token)
	case SignVersionNew:
		return checkSign(c, signCookie, SignVersionNew, token)
	default:
		return checkSign(c, signCookie, SignVersionOld, token)
	}
}
...
func checkSign(c *gin.Context, signCookie string, ver int, token string) bool {
	...
	if ver == SignVersionNew && ps.Get("guid") == "" {
	...
	if ver == SignVersionNew {
		signature = SignVersionNewPrefix + signature
	}
...
func getSignVersion(signStr string) int {
	if strings.Contains(signStr, SignVersionNewPrefix) {
		return SignVersionNew
	} else {
		return SignVersionOld
	}
}
