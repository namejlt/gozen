...
import (
	...
	"github.com/namejlt/gozen/util/runtime"
)
...
func TestExponentialBackoff(t *testing.T) {
	...
	if !errors.Is(err, ErrWaitTimeout) || i != opts.Steps {
...
func TestPollError(t *testing.T) {
	...
	fp := fakePoller{max: 1}
	if err := pollInternal(fp.GetWaitFunc(), f); err == nil || !errors.Is(err, expectedError) {
...
func TestPollImmediateError(t *testing.T) {
	...
	fp := fakePoller{max: 0}
	if err := pollImmediateInternal(fp.GetWaitFunc(), f); err == nil || !errors.Is(err, expectedError) {
		t.Fatalf("Expected error %v, got none %v", expectedError, err)
	}
	// We don't need to wait for fp.wg, as pollImmediate shouldn't call WaitFunc at all.
	used := atomic.LoadInt32(&fp.used)
